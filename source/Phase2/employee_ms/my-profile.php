...
if(isset($_POST['update'])){
    $stmt = $sql->prepare("select * from employees where email=? AND id!=?");
    $stmt->bindParam(1, $_POST['email'], PDO::PARAM_STR);
    $stmt->bindParam(2, $employeeid, PDO::PARAM_STR);
    $stmt->execute();
    $result = $stmt->fetch();
    if($stmt->rowCount()>0){
        ...
        $query = "UPDATE employees set firstname=?, lastname=?, phone=?, email=? where id=?";
...
if(isset($_POST['updatepassword'])){
    if($_POST['password']==$_POST['cpassword']){
        ...
        $stmt->bindParam(2, $employeeid, PDO::PARAM_INT);
        $stmt->execute();
                                    ...
                                    <div class="form-row">
                                       <div class="col-md-6 mb-3">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input required type="email" class="form-control" name="email" value="<?php echo $employee['email']; ?>">
                                        </div>
